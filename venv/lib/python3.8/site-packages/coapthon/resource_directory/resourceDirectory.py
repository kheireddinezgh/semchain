...
class ResourceDirectory(CoAP):
    ...
    """
    def __init__(self, host, port, database=MONGO_DATABASE, start_mongo=True, isDRD = False, DRD_ports = []):
        """
        ...
        self.add_resource('rd/', Registration(database=database))
        self.add_resource('rd-lookup/', Lookup())
        self.add_resource('rd-lookup/res', LookupRes(database=database, isDRD=isDRD, DRD_ports=DRD_ports))
