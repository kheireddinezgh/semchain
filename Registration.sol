...
contract Registration {
    ...
    function registerResource_DRDBC(address device, address drd_owner, string memory core_link_format, string memory uri) public {
        drds[drd_owner].push(device);
        devices_count++;
        resources[device].push(SharedStructs.Resource(uri, core_link_format));
    }

    function registerResource_DRDH(address device, address drd_owner, string memory core_link_format, string memory uri) public {
